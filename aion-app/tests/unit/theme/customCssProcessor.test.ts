...
describe('addImportantToAll', () => {
  ...
  it('does not corrupt selectors in a multi-rule skin-like snippet', () => {
    const css = `
:root { --primary: #165dff; --bg-base: #fff; }
body[arco-theme='dark'] .panel:hover { color: #fff; background: url(http://x/y.png); }
@media (max-width: 600px) { .c::before { content: "a; b: c"; } }
`;
    ...
    expect(declImportanceOf(processed).filter((d) => d.endsWith(':false'))).toEqual([]);
  });
});
