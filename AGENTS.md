...
This workspace hosts three related projects that together form the AionUi product stack.
...
Upstream product repos (reference): [AionUi](https://github.com/iOfficeAI/AionUi), [AionCore](https://github.com/iOfficeAI/AionCore), [OfficeCLI](https://github.com/iOfficeAI/OfficeCLI).
...
3. **Do not invent a fourth top-level package** or move shared code to the workspace root without an explicit user request.
...
- Do not push unless the user explicitly asks. In `aion-app` and `aion-core`, use `just push` instead of raw `git push`.
- Conventional Commits for app/core: `<type>(<scope>): <subject>`.
